...
class PrixFixtures extends Fixture
{
    ...
    {   
        ...
        $valeur=array(50,100,100,100,200,200,400,600,600,900,900,1000,1000,1500,1500,1500,1800,1800,2000,2700,2700,3000,3600,3600,3800,4600,6400,8000,8500,9900,11900,11900,13600,14500,21700,24500,31900,36000,37500);
        for($i=0;$i<count($borneInf);$i++){
            $tarif=new Tarif();
            $tarif->setBorneInferieure($borneInf[$i]);
            $tarif->setBorneSuperieure($borneSup[$i]);
